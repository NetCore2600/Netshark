...
: "${REMOTE:=no}"
: "${PROTOCHAIN:=yes}"
: "${LIBNETSHARK_TAINTED:=no}"
: "${LIBNETSHARK_CMAKE_TAINTED:=no}"
: "${MAKE_BIN:=make}"
...
if [ -z "$PREFIX" ]; then
    PREFIX=`mktempdir libnetshark_build`
...
# The norm is to compile without any warnings, but libnetshark builds on some OSes
...
tcc-*/*)
    ...
    LIBNETSHARK_TAINTED=yes
    ;;
clang-3.4/Linux-*)
    # netshark-netfilter-linux.c:427:10: error: will never be executed
    #   [-Werror,-Wunreachable-code]
    # netshark.c:3812:4: error: will never be executed
    #   [-Werror,-Wunreachable-code]
...
[ "$LIBNETSHARK_TAINTED" != yes ] && CFLAGS=`cc_werr_cflags`
...
# If necessary, set LIBNETSHARK_CMAKE_TAINTED here to exempt particular cmake from
# warnings. Use as specific terms as possible (e.g. some specific version and
# some specific OS).

[ "$LIBNETSHARK_CMAKE_TAINTED" != yes ] && CMAKE_OPTIONS='-Werror=dev'
...
run_after_echo "$PREFIX/bin/netshark-config" --help
run_after_echo "$PREFIX/bin/netshark-config" --version
run_after_echo "$PREFIX/bin/netshark-config" --cflags
run_after_echo "$PREFIX/bin/netshark-config" --libs
run_after_echo "$PREFIX/bin/netshark-config" --additional-libs
run_after_echo "$PREFIX/bin/netshark-config" --libs --static
run_after_echo "$PREFIX/bin/netshark-config" --additional-libs --static
run_after_echo "$PREFIX/bin/netshark-config" --libs --static-netshark-only
run_after_echo "$PREFIX/bin/netshark-config" --additional-libs --static-netshark-only

[ "$REMOTE" = yes ] && print_so_deps "$PREFIX/sbin/rnetsharkd"
[ "$REMOTE" = yes ] && run_after_echo "$PREFIX/sbin/rnetsharkd" -h
